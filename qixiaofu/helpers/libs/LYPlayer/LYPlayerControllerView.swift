...
class LYPlayerControllerView: UIView {
    ...
    var playerModel = LYPlayerModel(){
        didSet{
            ...
            let name = (playerModel.videoURL as NSString).lastPathComponent
            if ZFCommonHelper.isExistFile(LYPlayer.FILE_PATH(name)){
                self.freeLbl.isHidden = false
            }
        }
    }
    ...
    var urlAsset : AVURLAsset?{
        didSet{
            if urlAsset != nil{
                ...
                let cmTime = CMTime.init(value: CMTimeValue(0), timescale: 1)
                self.imageGenerator?.generateCGImagesAsynchronously(forTimes: [NSValue.init(time: cmTime)], completionHandler: { (requestedTime, im, actualTime, result, error) in
                    if im != nil{
                        self.playerModel.videoImage = UIImage.init(cgImage: im!)
                    }
                })
            }
        }
    ...
    /** 免流量提示 */
    fileprivate var freeLbl = UILabel()
    /** 锁定屏幕方向按钮 */
...
//            make.top.equalTo(self.snp.top).offset(22)
        ...
        self.freeLbl.snp.makeConstraints { (make) in
            make.trailing.equalTo(self.snp.trailing).offset(-15)
            make.bottom.equalTo(self.bottomView.snp.top).offset(15)
    ...
    func setUpDefaultViewAttribute() {
        ...
        self.freeLbl.font = UIFont.systemFont(ofSize: 13.0)
        self.freeLbl.textColor = BG_Color
        self.freeLbl.textAlignment = .right
        self.freeLbl.text = "免流量中..."
...
//        self.backBtn.isHidden = !self.fullScreenBtn.isSelected
        if self.delegate != nil{
...
//        }
        let videoUrl = self.playerModel.videoURL as NSString
        ZFDownloadManager.shared().downFileUrl(self.playerModel.videoURL, filename: videoUrl.lastPathComponent, fileimage: self.playerModel.videoImage)
...
//        }
    ...
    func setUpHideShowTimer() {
        ...
        if #available(iOS 10.0, *) {
            ...
            self.hideTimer = Timer.init(timeInterval: 5.0, target: self, selector: #selector(LYPlayerControllerView.timerAction), userInfo: nil, repeats: true)
            self.hideTimer?.fire()
        }
    ...
    func transferSecToMin(second:NSInteger) -> String {
        let min = second / 60
        let sec = second % 60
        var minStr = String.init(format: "%d", min)
        var secStr = String.init(format: "%d", sec)
        if min < 10{
            minStr = "0" + minStr
        }
        if sec < 10{
