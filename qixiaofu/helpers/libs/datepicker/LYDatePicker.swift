...
class LYDatePicker: UIControl {
    ...
    typealias ThreeComponentWithComponentBlock = (Date,NSInteger,NSInteger,NSInteger,Int) -> Void
    var ly_datepickerWithComponent : ThreeComponentWithComponentBlock?

    var dismissBlock : (() -> Void)?
    
    
    fileprivate lazy var timeTypeView : UIView = {
        let timeTypeView = UIView(frame:CGRect(x:0,y:kScreenH - 230 - 44 - 44,width:kScreenW,height:44))
        timeTypeView.backgroundColor = UIColor.white
        timeTypeView.isHidden = true
        return timeTypeView
    }()
...
extension LYDatePicker{
    ...
    func setUpGlobalViews() {
        ...
        //4.选择时间类型
        self.addSubview(self.timeTypeView)
        let titleArray = ["按照年","按照月","按照日"]
        
        for i in 0...2 {
            let btn = UIButton(frame:CGRect.init(x: kScreenW/3.0 * CGFloat(i), y: 0, width: kScreenW/3.0, height: 44))
            btn.tag = i
            btn.setTitle(titleArray[i], for: .normal)
            btn.setTitleColor(UIColor.RGB(r: 0, g: 122, b: 255), for: .normal)
            btn.titleLabel?.font = UIFont.systemFont(ofSize: 14)
            btn.addTarget(self, action: #selector(LYDatePicker.topBtnAction(btn:)), for: .touchUpInside)
            self.timeTypeView.addSubview(btn)
        }
    ...
    @objc func sureAction() {
        ...
        if (self.ly_datepickerWithComponent != nil) {
            self.ly_datepickerWithComponent!(self.date,self.year,self.month,self.day,self.component)
        }
    ...
    func showWithTimeType() {
        UIApplication.shared.keyWindow?.addSubview(self)
        UIApplication.shared.keyWindow?.bringSubview(toFront: self)
        
        self.subView.y = kScreenH
        self.timeTypeView.y = kScreenH - 44
        self.timeTypeView.isHidden = false
        self.addSubview(self.subView)
        UIView.animate(withDuration: 0.25, animations: {
            self.subView.y = kScreenH - 230 - 44
            self.timeTypeView.y = kScreenH - 230 - 44 - 44
        })
    }
