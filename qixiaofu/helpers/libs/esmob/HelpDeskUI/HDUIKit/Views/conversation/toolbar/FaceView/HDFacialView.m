...
//            _hdImageButton.imageView.contentMode = UIViewContentModeScaleAspectFit;
            ...
            [_hdImageButton setTitle:[HDConvertToCommonEmoticonsHelper convertToSystemEmoticons:emotion.emotionId] forState:UIControlStateNormal];
...
- (void)didSendEmotion:(HDEmotion *)emotion
{
    if (emotion) {
        if (emotion.emotionType == HDEmotionDefault || emotion.emotionType == HDEmotionPng) {
            if (_delegate) {
                NSString *didReceiveText = [HDConvertToCommonEmoticonsHelper convertToSystemEmoticons:emotion.emotionId];
                [_delegate selectedFacialView:didReceiveText];
